...
<section class="page-section bg-dark">
	...
	<div class="container">
		<div class="col-lg-12">
			...
			$cat = $connection->query("SELECT * FROM room_categories");
			$cat_arr = array();
			while ($row = $cat->fetch_assoc()) {
				$cat_arr[$row['id']] = $row;
			}
			$qry = $connection->query("SELECT distinct(category_id),category_id from rooms where id not in (SELECT room_id from checked where '$date_in' BETWEEN date(date_in) and date(date_out) and '$date_out' BETWEEN date(date_in) and date(date_out)  )");
